...
namespace EFcrud
{
    public class StripManager
    {
        ...
        public void ToonStripsFilter()
        {
            using (StripsContext ctx = new StripsContext())
            {
                foreach (Strip x in ctx.Strips.Include(x=>x.Reeks).Include(x=>x.Uitgever).Where(s=>s.Reeks.Naam=="Thorgal"))
                {
                    x.ToonDetail();
                    Console.WriteLine("-----------------------------------");
                }
        ...
        public void UpdateStrip()
        {
            using (StripsContext ctx = new StripsContext())
            {
                Strip strip = ctx.Strips.Single(x => x.StripID == 6);
        ...
        public void VerwijderStrip()
        {
            using (StripsContext ctx = new StripsContext())
            {
                ...
                ctx.Strips.Remove(new Strip() { StripID = 7 });
                ctx.SaveChanges();
            }
        }
    }
